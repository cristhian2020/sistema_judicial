...
interface ProcesoFormProps {
  proceso?: ProcesoIncidente;
  onSubmit: (proceso?: ProcesoIncidente) => void;
  onCancel: () => void;
  isEdit?: boolean;
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (isEdit && proceso?.id) {
        await updateDoc(doc(db, 'incidente', proceso.id), procesoData);
      } else {
        await addDoc(collection(db, 'incidente'), procesoData);
      }

      onSubmit(procesoData as ProcesoIncidente);
      setNurej('');
      setDemandados('');
      setDemandantes('');
      setCi('');
