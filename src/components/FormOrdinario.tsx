...
interface ProcesoFormProps {
    proceso?: ProcesoOrdinario;
    onSubmit: (proceso?: ProcesoOrdinario) => void;
    onCancel: () => void;
    isEdit?: boolean;
...
export default function FormOrdinario({
  proceso,
  onSubmit,
  onCancel,
  isEdit = false,
  ...
  const handleSubmit = async (e: FormEvent) =>{
    ...
    try {
      ...
      if(isEdit && proceso?.id){
        await updateDoc(doc(db, 'ordinario', proceso?.id), procesoData);
      }else{
        await addDoc(collection(db, 'ordinario'), procesoData)
      }
      setNurej('');
      setDemandados('');
      setDemandantes('');
