...
export default function FormPreparatoria({
  proceso,
  onSubmit,
  onCancel,
  isEdit = false,
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if(isEdit && proceso?.id){
        await updateDoc(doc(db, 'preparatoria', proceso?.id), procesoData);
      }else{
        await addDoc(collection(db, 'preparatoria'), procesoData)
      }
      setNurej('');
      setDemandados('');
      setDemandantes('');
      setCi('');
      setFecha_ingreso('');
      setEstado_proceso('PROCEDIMIENTO ABREVIADO');
      setObservacion('');
    ...
    <form onSubmit={handleSubmit} className="space-y-4 p-4 bg-white rounded shadow-md text-gray-800">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="md:col-span-2">
          <label className="block text-sm font-bold text-gray-700 mb-2">Remision a Juzgado o Tribunal de Sentencia con Acusacion Formal</label>
