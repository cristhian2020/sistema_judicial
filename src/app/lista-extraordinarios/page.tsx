...
export default function ListaExtraordinariosPage() {
  const [procesos, setProcesos] = useState<ProcesoExtraordinario[]>([]);
  const [filteredProcesos, setFilteredProcesos] = useState<
    ProcesoExtraordinario[]
  >([]);
  ...
  const fetchProcesos = async () => {
    try {
      const querySnapshot = await getDocs(collection(db, "extraordinario"));
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteDoc(doc(db, "extraordinario", id));
  ...
  const handleExport = () => {
    ...
    link.setAttribute("download", "procesos_extraordinarios.csv");
  ...
  return (
    <div className="container mx-auto py-8">
      ...
      <h1 className="text-3xl font-bold mb-6 text-center text-gray-800">
        Lista de Procesos Extraordinarios
      </h1>
              ...
              >
                ...
                ${
                  proceso.estado_proceso === "SENTENCIA"
                    ...
                    ? "bg-red-100 text-red-800"
                    : proceso.estado_proceso === "EXTINCION POR INACTIVIDAD"
                    ? "bg-red-100 text-red-800"
                    : proceso.estado_proceso === "RECHAZADAS"
                    ? "bg-red-100 text-red-800"
                    : proceso.estado_proceso === "POR NO PRESENTADADAS"
