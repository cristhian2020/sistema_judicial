...
export default function EditIncidentesPage({ params }: PageProps) {
  const router = useRouter();
  const [proceso, setProceso] = useState<ProcesoIncidente | null>(null);
  ...
  const fetchProceso = async (id: string) => {
    try {
      const docRef = doc(db, 'incidente', id);
