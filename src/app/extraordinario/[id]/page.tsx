...
export default function EditExtraordinarioPage({ params }: PageProps) {
  const router = useRouter();
  const [proceso, setProceso] = useState<ProcesoExtraordinario | null>(null);
  ...
  const fetchProceso = async (id: string) => {
    try {
      const docRef = doc(db, 'extraordinario', id);
      ...
      if (docSnap.exists()) {
        ...
        router.push('/lista-extraordinarios');
      }
    } catch (error) {
