...
export default function ListaPreparatoriaPage() {
  ...
  const fetchProcesos = async () => {
    try {
      const querySnapshot = await getDocs(collection(db, "preparatoria"));
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteDoc(doc(db, "preparatoria", id));
  ...
  const handleExport = () => {
    const headers = [
      "NUREJ,Tipo,Demandantes,Demandados,Estado,Fecha Ingreso,Cuerpos,Fojas,Imputacion,Observaciones",
    ];
    ...
    link.setAttribute("download", "procesos_preparatoria.csv");
  ...
  return (
    <div className="container mx-auto py-8">
      ...
      <h1 className="text-3xl font-bold mb-6 text-center text-gray-800">
        Lista de Procesos Preparatoria
      </h1>
          ...
          <tbody>
            {filteredProcesos.map((proceso) => (
              ...
              >
                ...
                ${
                  proceso.estado_proceso === 'PROCEDIMIENTO ABREVIADO'
                    ? "bg-green-100 text-green-800"
                    : proceso.estado_proceso === "CRITERIO DE OPORTUNIDAD"
                    ? "bg-red-100 text-red-800"
                    : proceso.estado_proceso === "SUSPENCION CONDICIONAL DEL PROCESO"
                    ? "bg-yellow-100 text-yellow-800"
                    : proceso.estado_proceso === "CONCILIACION"
                    ? "bg-red-100 text-red-800"
                    : proceso.estado_proceso === "SOBRESEIMIENTO"
                    ? "bg-red-100 text-red-800"
                    : proceso.estado_proceso === "EXTINCION DE LA ACCION EN ETAPA PREPARATORIA"
                    ? "bg-red-100 text-red-800"
                    : proceso.estado_proceso === "EXTINCION DE LA ACCION POR DURACION MAXIMA"
                    ? "bg-red-100 text-red-800"
                    : proceso.estado_proceso === "DECLINATORIA E INHIBITORIA"
                    ? "bg-red-100 text-red-800"
