...
export default function ListaOrdinariosPage() {
  const [procesos, setProcesos] = useState<ProcesoOrdinario[]>([]);
  ...
  const fetchProcesos = async () => {
    try {
      const querySnapshot = await getDocs(collection(db, "ordinario"));
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteDoc(doc(db, "ordinario", id));
  ...
  const handleExport = () => {
    ...
    link.setAttribute("download", "procesos_ordinarios.csv");
