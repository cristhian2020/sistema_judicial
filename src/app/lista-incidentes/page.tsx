...
export default function ListaIncidentesPage() {
  const [procesos, setProcesos] = useState<ProcesoIncidente[]>([]);
  ...
  const fetchProcesos = async () => {
    try {
      const querySnapshot = await getDocs(collection(db, "incidente"));
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteDoc(doc(db, "incidente", id));
  ...
  const handleExport = () => {
    ...
    link.setAttribute("download", "procesos_incidentes.csv");
