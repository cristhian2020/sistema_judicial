...
export default function ListaPreliminarPage() {
  ...
  const fetchProcesos = async () => {
    try {
      const querySnapshot = await getDocs(collection(db, "preliminar"));
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteDoc(doc(db, "preliminar", id));
  ...
  const handleExport = () => {
    ...
    link.setAttribute("download", "procesos_preliminares.csv");
